...
We will see a quick setup to configure the ``drone_docker`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``drone_docker`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``drone_docker`` formulas under ``/srv/salt/drone_docker/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/drone_docker/parameters/dns:domain/
    ln -s dns:domain /srv/salt/drone_docker/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/drone_docker/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/drone_docker/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``drone_docker`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``drone_docker`` formulas under ``/srv/salt/drone_docker/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/drone_docker/parameters/roles
...
We create a configuration for the role ``drone_docker/server`` in ``/srv/salt/drone_docker/parameters/roles/drone_docker/server.yaml``:
...
We create another configuration for the role ``drone_docker/client`` in ``/srv/salt/drone_docker/parameters/roles/drone_docker/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles drone_docker/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles drone_docker/client
...
.. code-block:: console
    ...
    mv /srv/salt/drone_docker/parameters/dns:domain/  '/srv/salt/drone_docker/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-drone_docker-as-json:
      test.nop:
        - name: {{ drone_docker | json }}
...
Here is an example based on `template-formula/drone_docker/config/file.sls`_:
...
.. code-block:: sls
    ...
    drone_docker-config-file-file-managed:
      file.managed:
        - name: {{ drone_docker.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='drone_docker-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ drone_docker.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``drone_docker`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ drone_docker['winner'] }}
...
.. _template-formula/drone_docker/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/drone_docker/config/file.sls
